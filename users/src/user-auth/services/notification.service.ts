...
export class NotificationService {
    ...
    async getNotiByUser(idUserTo: string) {
        const openedFalse = false
        const notis = await this.notiRespository.createQueryBuilder('noti')
            .leftJoinAndSelect('noti.idUserTo', 'user')
            .leftJoinAndSelect('noti.idUserFrom', 'userFrom')
            .where('user.idUser = :idUserTo', { idUserTo })
            .select(['noti', 'user','userFrom'])
            .getMany()

        const notisFalse = await this.notiRespository.createQueryBuilder('noti')
                                .leftJoinAndSelect('noti.idUserTo', 'user')
                                .leftJoinAndSelect('noti.idUserFrom', 'userFrom')
                                .where('user.idUser = :idUserTo', { idUserTo })
                                .andWhere('noti.opened = :openedFalse',{openedFalse})
                                .select(['noti', 'user','userFrom'])
                                .getMany()
        return {
            isSuccess: true,
